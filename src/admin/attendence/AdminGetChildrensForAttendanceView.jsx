...
import {  getChildrens, getChildrensByAdminAttendance, getChildrensRoleAdmin, getTeachersRoleAdmin, paySalaryAdmin } from "../../api/admin";
import { errorToast, successToast } from "../../toast";
import Spinner from "../../spinner";
import { IoMdClose } from "react-icons/io";

function AdminGetChildrensForAttendanceView() {
  ...
  const fetchAPI = async () => {
    try {
      setLoading(true);
      const response = await getChildrensByAdminAttendance();
  ...
  const goToParentDir = `/admin/admin-children-lists`;
...
<ul className={breadCrumbContainer}>
        ...
        <li className={`${breadCrubmberClassName}`}>
          <Link to={goToParentDir}>Childrens</Link>
        </li>
      </ul>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap px-6 py-4">
                              <Link to={`/admin/view-childrens-attendance/${item._id}/${item.name}`} >
...
export default AdminGetChildrensForAttendanceView;
