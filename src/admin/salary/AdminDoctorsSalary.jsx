...
import { getDoctorsRoleAdmin, getDoctorsStatusFalse, getTeachersRoleAdmin, paySalaryAdmin } from "../../api/admin";
...
const [modalData,setModalData] = useState();
  ...
  const handleSubmitPaySalary =async (e)=>{
    try {
        ...
        if(!date) return errorToast('Date is required')
        await paySalaryAdmin('doctor',{receiverId:modalData?._id},{date:date,amount:amount});
  ...
  const goToParentDir = `/admin/admin-doctors-salary`;
              ...
              <div className="overflow-hidden">
                    ...
                    {data.map((item, index) => {
                      return (
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap px-6 py-4">
                              <Link to={`/admin/admin-doctor-salary/${item._id}/${item.name}`} state={item} >
                ...
                {   <div className="">
                  ...
                  <div className="flex justify-end">
                    <h1 className="cursor-pointer mb-4 break-words text-2xl">
                      {" "}
                      <IoMdClose onClick={()=>{
                        setIsModalToggle(!isModalToggle)
