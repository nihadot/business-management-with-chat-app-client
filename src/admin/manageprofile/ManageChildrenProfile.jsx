...
import { deleteChildrenById, getChildrensRoleAdmin, updateStatus } from "../../api/admin";
...
function ManageTeachersProfile() {
  ...
  const fetchAPI = async () => {
    try {
      setLoading(true);
      const response = await getChildrensRoleAdmin();
  ...
  const handleApprovedOrReject = async(id,status)=>{
    ...
    try {
      ...
      if(status === 'false'){
        ...
        await updateStatus('children',id,Boolean(status));
        setRefresh(!refresh)
      } catch (error) {
        errorToast(
  ...
  const handleDelete = async(id)=>{
    try {
      ...
      const response =  await deleteChildrenById(id);
