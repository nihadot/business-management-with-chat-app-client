...
import { deleteDoctorById,getDoctorsRoleAdmin, updateStatus } from "../../api/admin";
...
function ManageTeachersProfile() {
  ...
  const fetchAPI = async () => {
    try {
      setLoading(true);
      const response = await getDoctorsRoleAdmin();
  ...
  const handleApprovedOrReject = async(id,status)=>{
    ...
    try {
      ...
      if(status === 'false'){
        ...
        await updateStatus('doctor',id,Boolean(status));
        setRefresh(!refresh)
      } catch (error) {
        errorToast(
