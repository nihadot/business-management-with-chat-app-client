...
import { getChildrensByParentId } from "../api/admin";
import Modals from "../components/Modals";
import { getChildrensAllByStatusTrueOnly, getChildrensAllByStatusaLL } from "../api/teacher";
...
function UserChildren() {
  ...
  const fetchAPI = async () => {
    try {
      setLoading(true);
      const response = await getChildrensAllByStatusaLL()
    ...
    <div className="">
      <div className="relative m-auto  shadow-md sm:rounded-lg mt-6 m-full md:w-[900px] overflow-x-scroll">
        <Link to={"/add-children"}>
          <button className="mb-3 mt-4 px-5 py-3 rounded-md bg-slate-700 text-white">
            Add Children
          </button>
        </Link>
        ...
        <div className="flex flex-col">
          <div className="overflow-x-auto max-h-[500px] sm:-mx-6 lg:-mx-8">
                    ...
                    {data.map((item, index) => {
                      return (
                        ...
                        >
                          ...
                          <td className="overflow-x-scroll max-w-44 whitespace-nowrap px-6 py-4">
                            {item.isStatus === true ? 'Approved' : 'Rejected'}
                          </td>
