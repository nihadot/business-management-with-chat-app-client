...
import { loginDoctor } from "../api/admin"
...
function DoctorLogin() {
    ...
    const handleSubmit = async(e)=>{
        ...
        try {
            ...
            if(response.status){
                localStorage.setItem("doctor-token",response.token)
                localStorage.setItem("doctor-details",JSON.stringify(response.result))
            }
            navigate('/doctors')
                ...
                <div className="p-5 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form onSubmit={handleSubmit}  className="space-y-4 md:space-y-6" action="#">
                        ...
                        <p className="text-sm font-light text-black dark:text-black">
                            Don’t have an account yet? <Link to={'/doctor-register'} className="font-medium text-primary-600 hover:underline dark:text-primary-500">Sign up</Link>
                        </p>
                    </form>
                </div>
            </div>
